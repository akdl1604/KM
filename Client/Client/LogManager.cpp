...
//	}
	...
	buffer.Format(_T("%s\n"), t.Format("Connected! - %Y-%m-%d %H:%M:%S"));
...
void LogManager::LogDisconnect(LPCSTR  mesg, ...)
{
	...
	t = CTime::GetCurrentTime();
	buffer.Format(_T("%s\n"), t.Format("Disconnected! - %Y-%m-%d %H:%M:%S"));
	va_start(argptr, mesg);
	for (int i = 0; i < format.GetLength(); i++) {
		if (format.GetAt(i) != '%') {
			buffer2 = " ";
...
void LogManager::Send(LPCSTR  mesg, ...)
{
	CStdioFile* pFile = this->logFile;
	CString buffer, buffer2, format;
	CTime t;
	...
	t = CTime::GetCurrentTime();
	buffer.Format(_T("%s\n"), t.Format("Send Data! - %Y-%m-%d %H:%M:%S"));
	va_start(argptr, mesg);
	for (int i = 0; i < format.GetLength(); i++) {
		if (format.GetAt(i) != '%') {
			buffer2 = " ";
...
CStdioFile * LogManager::Open()
{
	...
	fileName.Format(_T("%sÅë½ÅLog-%s.csv"), AfxGetAppName(), t.Format("%Y%m%d"));
